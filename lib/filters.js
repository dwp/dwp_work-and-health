 ...
 */
filter.trC = function trC(s, ss, r, f) {
 ...
 * @method upperC
 * @param  {String} s string containing character to be converted
 * @param  {Number} p position of the character
 * @return {String}   converted string
 */
filter.upperC = function upperC(s,p) {
  return s.replace(/\w\S*/g, function(txt){return txt.charAt(p ? p : 0).toUpperCase() + txt.substr(p ? p + 1 : 1).toLowerCase();});
}
